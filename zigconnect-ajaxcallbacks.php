...
if (!class_exists('ZigConnectAjaxCallBacks')) {



	final class ZigConnectAjaxCallBacks {
					...
					switch ($objField->zc_field_type) {
						case 'CHECKBOX' :
							...
							if (is_numeric($zc_data_id)) {
								$zc_data_value = $objZigConnect->GetData($zc_data_id)->zc_data_value;
							}
							?>
							<input class="zc_field_<?php echo $objField->zc_field_name?>" type="checkbox" name="zigconnect_metabox_data[<?php echo $zc_field_id?>][<?php echo $intOtherPostID?>]" value="1" <?php if ($zc_data_value == '1') { echo('checked="checked"'); } ?> />
							<?php
	...
	wp_die('Namespace clash! Class ZigConnectAjaxCallBacks already exists.');
